...
- The architecture, CPL, policy-engine, optimizer, Daml-boundary, Quickstart-foundation, first Quickstart workflow-orchestration layer, and first reference-adapter packages now provide executable evidence, though broader adapter coverage and workflow-coupled reference-data evidence are still pending.
- The `Demo artifacts` category now includes the end-to-end margin-call, return, and substitution reports plus their supporting policy, optimization, workflow, conformance, final demo-pack, prompt evidence artifacts, the Quickstart-backed package deployment proof, the first seeded Quickstart scenario proof, the first Quickstart-backed reference token adapter execution artifact, and the first Quickstart-backed end-to-end margin-call chain evidence.

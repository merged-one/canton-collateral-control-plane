...
| ADAPT-001 | Adapter-boundary discipline | Asset adapters may consume `SettlementInstruction` and related control contracts, execute asset-side actions, and emit machine-readable receipts, but they must not reinterpret policy, bypass workflow approvals, or authoritatively mutate obligation, encumbrance, or settlement workflow state off-ledger. | ADR 0018, ADR 0019, ADR 0020, `daml/CantonCollateral/ReferenceToken.daml`, `daml/CantonCollateral/QuickstartAdapter.daml`, `daml/CantonCollateral/QuickstartMarginCall.daml`, `daml/CantonCollateral/QuickstartSubstitution.daml`, `make localnet-run-token-adapter`, `make localnet-adapter-status`, `make demo-margin-call-quickstart`, `make demo-substitution-quickstart`, adapter execution reports, Prompt 15 evidence, Prompt 16 evidence, Prompt 17 evidence |
...
| CTRL-001 | Encumbrance and release control | If a policy or control state requires secured-party or pledgee approval for release, the release path must remain blocked until that approval is present and attributable. | Daml workflow modules, Daml test plan, authorization tests, `make demo-return`, `make demo-substitution`, `make demo-substitution-quickstart`, return and substitution reports, Quickstart substitution status artifacts |
...
| ATOM-001 | Atomic substitution and return | Collateral substitution and collateral return must complete atomically so that exposure coverage is not broken by intermediate visible states. | ADR 0007, ADR 0012, ADR 0013, ADR 0020, substitution and return workflow templates, Daml lifecycle scripts, `make demo-return`, `make demo-substitution`, `make demo-substitution-quickstart`, `make test-conformance`, conformance report, Quickstart substitution adapter and status artifacts |
...
| REPT-001 | Report fidelity | Every machine-readable execution report, return report, or substitution report must correspond exactly to committed workflow state and must not invent or omit materially relevant actions. | execution, return, and substitution report schemas and specs, Daml workflow result, Quickstart workflow result, adapter execution report, `make demo-margin-call`, `make demo-margin-call-quickstart`, `make demo-return`, `make demo-substitution`, `make test-conformance`, `make demo-all`, conformance report, final demo pack, demo evidence |
...
| WF-001 | Workflow authority | Obligation, encumbrance, approval, and settlement state may change only through committed workflow transitions on Canton; off-ledger services may propose or report changes but may not authoritatively apply them. | `daml/CantonCollateral/*.daml`, ADR 0007, ADR 0019, ADR 0020, Daml test plan, execution evidence, `make localnet-seed-demo`, `make localnet-run-margin-call-workflow`, `make demo-margin-call-quickstart`, `make demo-substitution-quickstart`, Quickstart seed receipts and workflow results |
| RUNTIME-001 | Runtime and demo separation | LocalNet overlays, demo bootstrap data, and runtime services must not alter policy semantics, bypass approvals, or fabricate successful reports. | deployment model, ADR 0015, ADR 0016, ADR 0017, ADR 0019, ADR 0020, Quickstart README, LocalNet control-plane runbook, `make localnet-smoke`, `make localnet-build-dar`, `make localnet-deploy-dar`, `make localnet-start-control-plane`, `make localnet-seed-demo`, `make localnet-status-control-plane`, `make demo-margin-call`, `make demo-margin-call-quickstart`, `make demo-return`, `make demo-substitution`, `make demo-substitution-quickstart`, `make demo-all`, prompt evidence |

...
| TOOL-001 | Toolchain reproducibility | Build, validation, and smoke-run commands must resolve to pinned Daml, Java, validation-tool, and Quickstart versions that are documented, checksum-verified or commit-pinned where downloaded, reproducible from a clean checkout, and explicitly tiered so portable verification does not silently depend on optional Docker-backed runtime checks. | dependency policy, bootstrap script, Quickstart bootstrap script, `scripts/verify-portable.sh`, `make verify-portable`, `make verify`, `make daml-build`, `make daml-test`, `make demo-run`, `make localnet-smoke`, verification commands |

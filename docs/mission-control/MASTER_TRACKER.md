...
Current Phase: Milestone 4 / Phase 4 - Initial Margin Call And Substitution Demo Reporting
...
Milestone 4 / Phase 4 now establishes the first executable bridge from policy evaluation and optimization into authoritative Daml workflow paths. The expected output is a reproducible margin-call and substitution demo set that consumes declared scenario inputs, emits machine-readable operator artifacts, and proves positive plus negative paths without pretending the full Quickstart deployment is complete.
...
2. Specify role-scoped `ExecutionReport` and `SubstitutionReport` disclosure profiles beyond the current operator-facing demo aggregates and workflow-party report baseline.
...
4. Define the first workflow-coupled optimizer reservation and consent interface, including substitution-scope carriage, without collapsing Canton authority.
...
- [ ] end-to-end return demo command

...
| `CollateralInventoryLot` | operational entity | a specific position or lot owned or controlled by a provider and available for potential collateral use | `lot_id`, `asset_id`, owner party, custodian, account, quantity, availability window, provenance, control method | smallest unit C-COPE uses for selection, encumbrance, substitution, and return |
